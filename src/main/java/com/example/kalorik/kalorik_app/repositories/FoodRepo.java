...
public interface FoodRepo extends JpaRepository<Food, Long> {
    ...
    @Query(value = "SELECT f.serving_size as servingsize, f.serving_unit as servingunit, f.id as id, f.name as name, f.calories as calories, f.bel as bel, f.fats as fats , f.ch as ch, mfi.quantity as quantity, mfi.unit as unit  FROM (meal_food_items mfi join meals m on mfi.meal_id=m.id) join food f on mfi.food_item_id=f.id WHERE meal_title=:mealTitle AND meal_date=:mealDate AND user_id=:uid", nativeQuery = true)
