...
public interface RecipeRepository extends JpaRepository<Recipes, Long> {
    ...
    @Query("SELECT r FROM Recipe r JOIN FETCH r.categories c WHERE c.id = :categoryId")
    List<Recipes> findByCategoryId(@Param("categoryId") Long categoryId);

    // Использование IN для поиска по нескольким продуктам
    @Query("SELECT DISTINCT r FROM Recipe r JOIN FETCH r.products p WHERE p.id IN :productIds")
    ...
    @Query("SELECT r FROM Recipe r JOIN FETCH r.categories c WHERE c.id IN :categoryIds " +
           "GROUP BY r.id HAVING COUNT(DISTINCT c.id) = :categoryCount")
